  ...
  class CLI
    class AssumeTest < AbidTest
      def test_run
        ...
        state1 = env.state_manager.states.where(name: 'foo').first
        assert_equal "---\n:a: 1\n:b: 2000-01-01\n", state1.params

        state2 = env.state_manager.states.where(name: 'bar').first
        assert_equal "---\n:a: 1\n:b: 2000-01-01\n", state2.params

        # run again
        Assume.new(env, {}, %w(foo a=1 b=2000-01-01)).run
        assert_equal 1, env.state_manager.states.where(name: 'foo').count
        ...
        assert env.state_manager.states.where(name: 'foo').first.successed?
      end
    end
  end
end
